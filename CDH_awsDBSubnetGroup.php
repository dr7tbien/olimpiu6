<?php
    ...
    */
    ...
    class CDH_awsDBSubnetGroup extends CDH_abstractAwsService {
        ...
        public function __construct(){
            ...
            $this->setClientRds();
            $this->setClientEc2();
            $this->setClassName("CDH_awsDBSubnetGroup");
        }
        ...
        public function create( $data = null ) {
            ...
            try{
                ...
                if ( $this->existsByName( $data['DBSubnetGroupName'] ) )
                    throw new Exception( "L044 El servicio " . $this->getClassName() . " con el nombre " . $data['DBSubnetGroupName'] . " ya existe");
                
                $result     = $client->createDBSubnetGroup( $data );
                $service    = $result['DBSubnetGroup'];

                if( $this->waitToBeCreated(  $service['DBSubnetGroupName']  ) == false )
                ...
                $this->logAdmin(  "L058 Error  " . $this->getClassName() . "->create()" );  
                $this->logAdmin(  "\n Mensaje de error:" );
                $this->logAdmin(  $e->getMessage() );
                return false;
            }
        ...
        public function getServiceByName( $name  ){
            ...
            if( !is_object($result) || !is_array( $result['DBSubnetGroups'] )  )
                return false;

            $services = $result['DBSubnetGroups'];

            foreach ( $services as $service ){
                #echo PHP_EOL . "\$service = " . $service['Name'] . " | \$name = $name";    
                if( strcmp ($service['DBSubnetGroupName'], $name ) == 0 ) 
                    return $service;
        ...
        public function delete( $name ){
            ...
            try{
                ...
                $result = $client->deleteDBSubnetGroup([
                    'DBSubnetGroupName' => $name, // REQUIRED
                ]);
                return true;
            }catch(AwsException $e){
                $e->getMessage();
        ...
        public function waitToBeCreated( $name, $seconds = null, $times = null ){
            ...
            for($i = 0; $i < $times;  $i++ ){
                ...
                #var_export( $result );
                #echo "\ncount(\$result['DBSubnetGroups']) = " . count($result['DBSubnetGroups']); 
                if( isset($result['DBSubnetGroups']) && is_array($result['DBSubnetGroups']) && count($result['DBSubnetGroups']) == 1 )
                    return true;
